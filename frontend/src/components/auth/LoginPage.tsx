...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (error === 'google_failed') toast.error('Failed to sign in with Google');
    if (error === 'google_not_configured') toast.error('Google OAuth is not configured');
    if (error === 'keycloak_failed') toast.error('Failed to sign in with SSO');
    if (error === 'keycloak_not_configured') toast.error('SSO is not configured on the server');
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-600 to-primary-500 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Project & Program Management</p>
        </div>
